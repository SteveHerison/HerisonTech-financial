...
export default function BudgetsCard() {
  const limit = 4;
  return (
    <div className="flex  flex-col gap-y-5 bg-zinc-800 rounded-xl flex-1">
      <div className="flex flex-col w-full md:flex-row h-full  gap-4">
        <BudgetTracker />

        <div className="flex w-full md:max-w-20 lg:max-w-32 xl:max-w-40 flex-row h-full items-center justify-between md:flex-col text-slate-50">
            ...
            {data.pots.slice(0, limit).map((item, index) => (
              <li key={index}>
                ...
                >
                  <span>
                    <p>{item.name}</p>
                    <strong className="text-slate-50 font-jetBrains text-sm md:">
