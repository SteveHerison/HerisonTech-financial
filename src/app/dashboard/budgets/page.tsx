  ...
  return (
    <section className="flex flex-col w-full h-full px-10 pt-8 pb-2  gap-y-4 containers overflow-y-auto">
      <div className="flex items-center justify-between">
        ...
        <button className="rounded-lg p-2 bg-primário hover:shadow-md hover:shadow-primário/50 font-semibold transition-all">
          +ADD
        </button>
      </div>

      <div className=" flex gap-2 w-full  flex-col lg:flex-row overflow-y-auto">
        <div className="flex bg-primário p-5 rounded-xl lg:w-2/5 flex-col sm:flex-row lg:flex-col max-h-[680px] sm:gap-y-20 sm:gap-x-10 lg:gap-y-0 ">
          <BudgetTracker />
            ...
            <ul className="flex flex-col h-full gap-4">
              {data.pots.map((item, index) => (
                <li key={index} className="h-full flex items-center">
                  ...
                  >
                    <span className="flex items-center w-full justify-between  space-x-8 ">
